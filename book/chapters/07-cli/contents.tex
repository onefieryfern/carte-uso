...
Astfel când shellul primește comanda \cmd{ls} și identifică executabilul \file{/bin/ls}, va crea un proces copil identic (tot shell) folosind \texttt{fork()} și apoi va înlocui imaginea de executabil cu \file{/bin/ls} folosind \texttt{exec()} așa cum am prezentat în \labelindexref{Figura}{fig:process:create}.
